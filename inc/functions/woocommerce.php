...
add_action('woocommerce_single_product_summary', 'lusion_category_mobile', 3);
...
add_action('woocommerce_before_single_product_summary', 'lusion_show_sale_percentage_loop', 10);
    ...
    if ($image_product): ?>
        <div class="image-product-sc">
            <?php if ($detect->isTablet()): ?>
                ...
                if (isset($woocommerce_loop['single_type']) && $woocommerce_loop['show_custom_image'] === 'yes'): ?>
                    ...
                    <?php if ($woocommerce_loop['show_custom_image'] === 'yes') : ?>
                        <?php
                        $image_url = Lusion_Helper::aq_resize(array(
                            'url' => $image_product,
            ...
            <?php else: ?>
                ...
                <?php endif; ?>
            <?php endif; ?>
            <?php lusion_show_sale_percentage_loop(); ?>
        </div>
    <?php endif;
}
function lusion_woocommerce_list_view()
{
    $lusion_product_columns = lusion_get_meta_value('product_column');
    $lusion_product_layout = '';
    ...
    if($lusion_product_columns == '1'){
        $lusion_product_layout = 'list';
    }else{
        $lusion_product_layout = 'grid';
    }
       if ($lusion_product_layout == 'list'){
         $list_change_col = Lusion::setting('product_column'); 
       }
            ...
            <?php if ($lusion_product_layout == 'grid'): ?>
                ...
                <li class="four-2">
                    <a class="active" href="#" id="grid4" data-layout="layout-grid"
                       data-column="<?php echo esc_attr($lusion_product_columns); ?>"><i class="theme-icon-grid"></i>
                    </a>
                </li>
                <li class="list-last">
                    <a class="" href="#" id="list1" data-layout="layout-list"
                       data-column="<?php echo esc_attr($lusion_product_columns); ?>"><i class="theme-icon-list"></i></a>
                </li>
            <?php endif; ?>
...
function lusion_show_sale_percentage_loop()
{
    global $product;
	$max_percentage = '';
    ...
    if ($max_percentage > 0) echo "<div class='sale_perc'>-" . round($max_percentage) . "%</div>";
}
        ...
        if ($product->is_on_sale()) {
            ...
            if ($regular_price !== $sale_price || $regular_price_max !== $sale_price_max && $product->is_on_sale()) {
                ...
                if ($percentage_price_max !== 0 && $percentage_price_min !== 0) {
                    $percentage = '<span class="label-upto">' . esc_html__('-', 'lusion') . '</span>' . $percentage_price_max;
                } elseif ($percentage_price_min > 0 && $percentage_price_max == 0) {
                    $percentage = esc_html__('-', 'lusion') . $percentage_price_min;
                } else {
                    $percentage = esc_html__('-', 'lusion') . $percentage_price_max;
                }
            }
            if ($product->get_regular_price())
                $percentage = -round((($product->get_regular_price() - $product->get_sale_price()) / $product->get_regular_price()) * 100);
            if (Lusion::setting('percentage_lable') && $percentage) {
                $percentage_price_min = round(($regular_price - $sale_price) / $regular_price * 100);
                $percentage_price_max = round(($regular_price_max - $sale_price_max) / $regular_price_max * 100);
    ...
    <?php if (isset($woocommerce_loop['product_type']) && $woocommerce_loop['show_custom_image'] === 'yes'): ?>
        ...
        <?php if (is_array( $attachment_ids ) && !empty($attachment_ids) ): ?>
            <?php if ($woocommerce_loop['show_custom_image'] === 'yes') : ?>
                ...
                $last_image_url = Lusion_Helper::aq_resize(array(
                    'url' => $full_image_size,
                    'width' => $attachment_size[0],
                    'height' => $attachment_size[1],
                    'crop' => true,
...
function lusion_single_gallery_nav()
{
    ...
    <div class="product-list-thumbnails">
        ...
        foreach ($gallery_ids as $key => $value) {
            ...
            $image_url_2 = Lusion_Helper::aq_resize(array(
                'url' => $full_image_size,
                'width' => 152,
                'height' => 205,
                'crop' => true,
...
function lusion_compare_product()
{
    ...
    if ((isset($woocommerce_loop['show_compare']) && $woocommerce_loop['show_compare'] === 'yes') && $compare) {
        ...
        if ($compare && Lusion::setting('product_compare')) {
            printf('<div class="action-item compare-product"><a data-toggle="tooltip" data-placement="top" href="%s" class="%s" data-product_id="%d"><i class="theme-icon-shuffle"></i></a><span class="tooltip-custom">' . esc_html__("Compare", "lusion") . '</span></div>', lusion_add_compare_action($product->get_id()), 'add_to_compare compare button', $product->get_id(), esc_html__('Compare', 'lusion'));
        }
    }
    ?>
...
function lusion_size_guide_link()
{
    ...
    <div class="size-guide-product">
        <a target="_blank" href="<?php echo esc_url($size_guide_link); ?>"><?php echo esc_html__('Size Guide', 'lusion') ?></a>
    </div>
<?php endif; ?>
    <?php
}
