...
function lusion_get_save_template(){
    $block_options = array();
    $args = array(
        'numberposts' => -1,
...
function lusion_get_id_by_slug_cm($slug){
    $id='';
    $args = array(
        'name'           => $slug,
        'post_type'      => 'elementor_library',
...
function lusion_get_icon_coming_soon(){
    $icon_coming_soon= [];
    $args = array(
        'post_type'      => 'elementor_library',
        'posts_per_page' => -1,
...
function lusion_get_id_by_slug($slug){
    $id='';
    $args = array(
        'name'           => $slug,
        'post_type'      => 'header',
...
function lusion_get_id_by_slug_footer($slug){
    $id='';
    $args = array(
        'name'           => $slug,
        'post_type'      => 'footer',
        ...
        <?php if ($audio && $audio != ''): ?>
            <?php if(is_singular()):?>
                <div class="blog-audio">
                        ...
                        ?>                    
                            <?php echo Lusion_Helper::w3c_iframe( wp_oembed_get( $audio,  array('height'=>300 ) )); ?>
                        <?php if(get_post_format() == 'audio'){
                            echo '</div>';
                        }
                        ?>                 
            ...
            <?php else:?>
                <div class="blog-audio">
                        ...
                        ?>                    
                            <?php echo Lusion_Helper::w3c_iframe( wp_oembed_get( $audio,  array('height'=>230 ) )); ?>
                        <?php if(get_post_format() == 'audio'){
                            echo '</div>';
                        }
                        ?>                 
...
function lusion_limit_title($numberlimit)
{
    ...
    if (strlen($tit) > $numberlimit) echo esc_html__('...', 'lusion');
}
...
/* $pluginPathEl =  'elementor/elementor.php';
$pluginPathAO =  'autoptimize/autoptimize.php';
if(is_plugin_active( $pluginPathEl ) && is_plugin_active( $pluginPathAO ) ): 
