...
<footer id="page-footer" class="page-footer footer-builder <?php echo esc_attr($footer_class);?>">
    <div class="footer-content">
        <?php
        if (!empty($footer_type) && $footer_type !=='default'){
        echo \Elementor\Plugin::$instance->frontend->get_builder_content(lusion_get_id_by_slug_footer($footer_type), true);
        }else{
            echo \Elementor\Plugin::$instance->frontend->get_builder_content(lusion_get_id_by_slug_footer($choose_footer_builder), true);
        }
        ?>
    </div>
</footer>