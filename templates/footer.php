...
if (class_exists( 'Apr_Core' )):
    if (isset($footer_type) && $footer_type == 'default'|| is_post_type_archive()=='portfolio') {
        if (Lusion::setting('enable_footer_builder') == '1') {
            ...
            echo \Elementor\Plugin::$instance->frontend->get_builder_content(lusion_get_id_by_slug($choose_footer_builder), true);
            echo '</div>';
        } else {
        ...
        if ($footer_type !==''){
            echo '<div class="footer-content">';
            echo \Elementor\Plugin::$instance->frontend->get_builder_content(lusion_get_id_by_slug($footer_type), true);
            echo '</div>';
        }else{
            get_template_part('templates/footer/footer-default');
        }
