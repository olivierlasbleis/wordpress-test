...
if (class_exists( 'Apr_Core' )):
    if (isset($header_type) && $header_type == 'default' || is_post_type_archive()=='portfolio') {
        if (Lusion::setting('enable_header_builder') == '1') {
            ...
            echo \Elementor\Plugin::$instance->frontend->get_builder_content(lusion_get_id_by_slug($choose_header_builder), true);
            echo '</div>';
        } else {
        ...
        if ($header_type !==''){
            echo '<div class="header-content">';
            echo \Elementor\Plugin::$instance->frontend->get_builder_content(lusion_get_id_by_slug($header_type), true);
            echo '</div>';
        }else{
            get_template_part('templates/header/header-default');
        }
